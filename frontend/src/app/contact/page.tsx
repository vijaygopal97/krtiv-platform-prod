  ...
  return (
    ...
    >
      ...
      <div className="relative z-[1]">
        ...
        <section className={`${SITE_HEADER_OFFSET_CLASS} pb-10 md:pb-14 pt-6 md:pt-8`}>
          <div className="max-w-[1200px] mx-auto px-6 md:px-10">
            <ScrollReveal>
              <h1
                className="display-xl text-balance tracking-wide uppercase"
                style={{ fontSize: 'clamp(2.25rem, 6vw, 5.25rem)' }}
              >
                <Editable cmsKey="contact.main.title" defaultValue="Contact Us" as="span" />
              </h1>
            </ScrollReveal>
          </div>
          <div className="max-w-[1200px] mx-auto px-6 md:px-10 grid md:grid-cols-12 gap-8 md:gap-10 mt-8">
            <ScrollReveal className="md:col-span-7">
              <form
                onSubmit={(e) => e.preventDefault()}
                className="bg-white rounded-[20px] border hairline p-8 md:p-10 shadow-[0_12px_40px_rgba(0,0,0,0.06)]"
              >
                <h2 className="font-display text-xl mb-6">Contact form</h2>
                <div className="grid sm:grid-cols-2 gap-5">
                  <label className="block">
                    <span className="block text-xs tracking-wide text-[color:var(--ink-soft)] mb-2">Name</span>
                    <input
                      type="text"
                      className="w-full h-12 px-4 rounded-xl bg-[color:var(--ivory)] border hairline outline-none focus:border-[color:var(--ink)] transition"
                    />
                  </label>
                  <label className="block">
                ...
                <label className="block mt-5">
                  ...
                  <textarea
                    rows={6}
                    className="w-full p-4 rounded-xl bg-[color:var(--ivory)] border hairline outline-none focus:border-[color:var(--ink)] transition resize-y"
                  />
                </label>
                <button
                  type="submit"
                  className="mt-7 inline-flex items-center gap-2 h-12 px-6 rounded-full bg-[color:var(--ink)] text-white text-sm"
                >
